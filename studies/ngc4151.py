...
def fit_hbeta_and_measure_z(spectrum, approx_hbeta_angstrom, R):
    ...
    sub_spec=sub_spec-np.mean(sub_spec.flux[:100].value)
    # Fit sécurisé
...
def plot_combined(spectrum_full, spectrum_zoom, approx_hbeta_angstrom=4876*u.AA, R=850):
    ...
    ax0.set_title(
        r'\textbf{NGC4151}' + '\n'
        r'2025/06/16 - La Montagne - G.Bertrand' + '\n'
    ...
    plt.tight_layout()
    plt.savefig("ngc4151.png", dpi=150, bbox_inches='tight')
    plt.show()
...
def plot_fit_result(ax, spectrum, model):
    ...
    lines = [r"Broad H$\beta$ component", r"Narrow H$\beta$ component", r"[OIII] $\lambda$4959", r"[OIII] $\lambda$5007", r"Continnuum"]
...
spectrum = Spectrum1D.read("C:\\Users\\g-ber\\Documents\\ASTRO\\starex\\ccdciel\\20250616\\_ngc4151_20250616_882.fits", format='wcs1d-fits')
spectrum_abs = Spectrum1D.read("C:\\Users\\g-ber\\Documents\\ASTRO\\starex\\ccdciel\\20250616\\_ngc4151_20250616_882-abs.fits", format='wcs1d-fits')

z, fitted_center, fit,fitter, spectrum_rest = plot_combined(spectrum_abs, spectrum, approx_hbeta_angstrom=4876*u.AA, R=850)
print(f"z: {z:.8f}")
...
log_M_BH, M_BH, d_l = calculate_mbh_vestergaard(flux, fwhm_intrinsic_kms, z,0.027)
log_M_BH2, M_BH2, d_l2 = calculate_mbh_feng(flux, fwhm_intrinsic_kms, z,0.027)
